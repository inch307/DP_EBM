...
parser.add_argument('--af_max_remove', default=1, type=int, help='at each af_epoch, #af_max_prune features can be removed at most')
...
def main():
    ...
    if os.path.exists('experiment.csv'):
        csv_f = open('experiment.csv', 'a', newline='')
        wr = csv.writer(csv_f)
    else:
        csv_f = open('experiment.csv', 'w', newline='')
        wr = csv.writer(csv_f)
        write_columns(wr)
