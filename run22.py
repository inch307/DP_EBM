...
def main():
    ...
    if args.data_path == 'syn_cls':
        X, y = datasets.make_classification(n_samples=10000, n_features=60, n_informative=5, n_redundant=5, n_clusters_per_class=2, random_state=2022)
        column_name = []
        for i in range(20):
    ...
    # print(df)
    if os.path.exists('experiment22.csv'):
        csv_f = open('experiment22.csv', 'a', newline='')
        wr = csv.writer(csv_f)
    else:
        csv_f = open('experiment22.csv', 'w', newline='')
        wr = csv.writer(csv_f)
        write_columns(wr)
